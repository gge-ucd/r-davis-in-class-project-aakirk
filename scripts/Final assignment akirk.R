...
#adding column with month, day, hour 
...
#1. Plot the departure delay of flights against the precipitation, and include a simple regression line as part of the plot. Hint: there is a geom_ that will plot a simple y ~ x regression line for you, but you might have to use an argument to make sure its a regular linear model. Use ggsave to save your ggplot objects into a new folder you create called plots.
...
#2. Create a figure that has date on the x axis and each days mean departure delay on the y axis. Plot only months September through December. Somehow distinguish between airline carriers (the method is up to you). Again, save your final product into the plot folder.
...
#4. Make a function that can: (1) convert hours to minutes; and (2) convert minutes to hours (i.e., its going to require some sort of conditional setting in the function that determines which direction the conversion is going). Use this function to convert departure delay (currently in minutes) to hours and then generate a boxplot of departure delay times by carrier. Save this function into a script called custom Functions.R in your scripts/code folder.
...
#5. Below is the plot we generated from the new data in Q4. (Base code: ggplot(df, aes(x = dep_delay_hrs, y = carrier, fill = carrier)) +   geom_boxplot()). The goal is to visualize delays by carrier. Do (at least) 5 things to improve this plot by changing, adding, or subtracting to this plot. The skys the limit here, remember we often reduce data to more succinctly communicate things.
##starting with:
